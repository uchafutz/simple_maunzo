...
<div class="body flex-grow-1 px-3">
        <div class="container-lg">
            ...
            <div class="col-12 d-flex justify-content-end mb-2">
                <a href="{{route('items.create')}}" class="btn label-btn btn-primary">
                              ...
                              <tbody>
                                @foreach($items as $item)
                                    <tr>
                                        ...
                                        <td>{{$item->code}}</td>
                                        <td>{{$item->stores->name ?? 'Null'}}</td>
                                        <td>{{$item->level}}</td>
                                        <td>{{$item->unities->name ?? 'Null'}}</td>
                                        <td>{{$item->sell_price}}</td>
                                        
                                        <td style="width: 80px;">
                                            <div class="hstack gap-2 fs-15 text-center">
                                                <a href="{{route('items.show', $item)}}" type="success" icon="ri-eye-2-line">show</a>
